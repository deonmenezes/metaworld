...
const ProductsGridcom = () => {
    ...
    <div className={styles.container}>
      {ProductsGrid.map((section, index) => (
        <div key={index} className="mb-6">
          <h2 className="text-2xl text-white font-bold mb-6 text-center ">{section.category}</h2>
          {section.pp && (
  
  <div className="flex justify-center mb-10">
  <p className="text-left text-white text-lg mb-4 sm:w-1/2 break-words">
    {section.pp}
  </p>
</div>
  ...
  {section.items.map((item, i) => (
    ...
    <div className="text-gray-700 text-sm text-center mx-auto" dangerouslySetInnerHTML={{ __html: item.description.replace(/\./g, ".<br/>") }} />
  </div>
  ))}
</div>
        </div>
